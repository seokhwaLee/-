...
register_coco_instances("carrs_data", {}, os.path.join(settings.BASE_DIR,'classification/inference/test_img/test_coco.json'), os.path.join(settings.BASE_DIR,'classification/inference/test_img'))


def front_car(media_root, file_name):
    ...
    if predicted[0] != 0:
        ...
        # damage_class_map= {1:'dent', 2:'scratch', 3:'emblem', 4:'lamp'}
        ...
        v = Visualizer(img[:, :, ::], metadata=MetadataCatalog.get("carrs_data"))
        img = v.draw_instance_predictions(ni).get_image()
        plt.imshow(img)
        plt.show()

        try :
            os.remove(settings.DAMAGE_ROOT+'/front.jpg')
        except :
            pass

        io.imsave(settings.DAMAGE_ROOT+'/'+ 'front.jpg', img)
