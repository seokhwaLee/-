...
register_coco_instances("carsss_data", {}, os.path.join(settings.BASE_DIR,'classification/inference/annotations.json'), os.path.join(settings.BASE_DIR,'classification/inference/'))


def left_car(media_root, file_name):
    ...
    if predicted[0] != 0:
        ...
        dataset = DatasetCatalog.get("carsss_data")
        metadata = MetadataCatalog.get("carsss_data")
        ...
        v = Visualizer(img[:, :, ::], metadata=MetadataCatalog.get("carsss_data"))
        img = v.draw_instance_predictions(ni).get_image()


        try :
            os.remove(settings.DAMAGE_ROOT+'/left.jpg')
        except :
            pass

        io.imsave(settings.DAMAGE_ROOT+'/'+ 'left.jpg', img)
