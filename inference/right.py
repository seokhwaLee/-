...
register_coco_instances("carssss_data", {}, os.path.join(settings.BASE_DIR,'classification/inference/annotations.json'), os.path.join(settings.BASE_DIR,'classification/inference/'))


def right_car(media_root, file_name):
    ...
    if predicted[0] != 0:
        ...
        dataset = DatasetCatalog.get("carssss_data")
        metadata = MetadataCatalog.get("carssss_data")
        ...
        v = Visualizer(img[:, :, ::], metadata=MetadataCatalog.get("carssss_data"))
        img = v.draw_instance_predictions(ni).get_image()


        try :
            os.remove(settings.DAMAGE_ROOT+'/right.jpg')
        except :
            pass

        io.imsave(settings.DAMAGE_ROOT+'/'+ 'right.jpg', img)
